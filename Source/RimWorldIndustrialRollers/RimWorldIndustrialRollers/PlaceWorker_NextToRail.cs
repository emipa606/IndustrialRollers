...
internal class PlaceWorker_NextToRail : PlaceWorker
{
        ...
        Thing thingToIgnore = null, Thing thing = null)
    {
        ...
        var northEast = (int)rot.AsAngle == 0 || (int)rot.AsAngle == 90;
            ...
            if (item.InBounds(map))
            {
                if (northEast &&
                    (map.zoneManager.ZoneAt(item) != null && map.zoneManager.ZoneAt(item) is Zone_Stockpile ||
                     ...
                     map.thingGrid.ThingAt(item, ThingCategory.Building) != null && map.thingGrid
                         .ThingAt(item, ThingCategory.Building).TryGetComp<CompRefuelable>() != null || item
                         .GetThingList(map).Any(x =>
                             x.def is { defName: { } } && x.def.defName.Contains("MovingRail") &&
                             !x.def.defName.Equals("MovingRailPusher") &&
                             !x.def.defName.Equals("MovingRailGreenPuller") &&
                ...
                if (!northEast && item.GetThingList(map).Any(x =>
                        x.def is { defName: { } } && x.def.defName.Contains("MovingRail") &&
                        !x.def.defName.Equals("MovingRailPusher") && !x.def.defName.Equals("MovingRailGreenPuller") &&
                        !x.def.defName.Equals("MovingRailPuller")))
                {
                    num++;
                }
        ...
        return num >= 2 ? true : "MovingRail_MustPlaceNextToRail".Translate();
    }
